...
def quadratic(a, b, c):
    ...
    >>> q_and_dq = quadratic(1, 6, 8) # x**2 + 6*x + 8
...
def mul_fns(f_and_df, g_and_dg):
    ...
    return lambda x, derive: f_and_df(x, derive) * g_and_dg(x, not derive) + f_and_df(x, not derive) * g_and_dg(x, derive) if derive == True else f_and_df(x, derive) * g_and_dg(x, derive)
...
def letters(tree):
    ...
    if is_huffman_leaf(tree):
        ...
        lst = []
        [lst.extend(letters(t)) for t in branches(tree)]
        return lst
...
def decode_one(tree, code):
    ...
    while l is None:
        num = code.pop(0)
        if num == 0:
            tree = branches(tree)[0]
            if is_huffman_leaf(tree):
                l = letter(tree)
        else:
            tree = branches(tree)[1]
            if is_huffman_leaf(tree):
                l = letter(tree)
    return l
...
def encodings(tree):
    ...
    """
    dic = {}
    def encode(t, parent, l):
        if letter(t) == l:
            dic[l] = parent
        if is_huffman_leaf(t):
            return None
        elif parent == None:
            b = [b for b in branches(t)]
            encode(b[0], [0], l)
            encode(b[1], [1], l)
        else:
            b = [b for b in branches(t)]
            encode(b[0], parent + [0], l)
            encode(b[1], parent + [1], l)
...
def huffman(frequencies):
    ...
    t =  huffman_tree(leaves.pop(0), leaves.pop(0))
    while len(leaves) > 0:
        t = huffman_tree(t, leaves.pop(0))
    return t
...
def squarer(a, b):
    ...
    c = make_ternary_constraint(a, a, b, mul, truediv, truediv)
    d = make_ternary_constraint(b, b, a, sqrt, square, square)
    return c
...
def pythagorean(a, b, c):
    ...
    def square_subtract(a,b):
        return (a**2 - b**2)**0.5

    adder_squared = make_ternary_constraint(a, b, c, square_add, square_subtract, square_subtract)
