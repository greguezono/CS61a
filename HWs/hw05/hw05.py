...
def replace_leaf(t, old, new):
    ...
    for branch in branches(t):
        branchs.append(replace_leaf(branch, old, new))
...
def move_stack(n, start, end):
    ...
    if n == 1:
        ...
        open_pole = 6 - start - end
        move_stack(n-1, start, open_pole)
        print_move(start, end)
        move_stack(n-1, open_pole, end)
...
def is_weight(w):
    ...
    return is_leaf(w) and type(size(w)) == int
...
def balanced(m):
    ...
    len1 = length(side1)
    len2 = length(side2)
    weight1 = total_weight(end(side1))
    weight2 = total_weight(end(side2))
    torque1 = len1 * weight1
    torque2 = len2 * weight2
    if torque1 != torque2:
        return False
    else:
        return balanced(end(side1)) and balanced(end(side2))
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, password_attempt):
        nonlocal balance, number_of_attempts, password_attempts
