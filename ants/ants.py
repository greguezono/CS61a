...
class ThrowerAnt(Ant):
	...
	def nearest_bee(self, hive):
		...
		def near_bee(place):
			...
			if place == hive:
				return None
			elif steps > self.max_range:
				return None
			elif place.bees and steps >= self.min_range:
				return random_or_none(place.bees)
			else:
				steps += 1
				return near_bee(place.entrance)
		return near_bee(self.place)
...
# END Problem 13
	...
	def action(self, colony):
		...
		"""
		# BEGIN Problem 13
		"""kill queen ant if there is aleardy a queen ant"""
		if not self.first_queen:
			self.reduce_armor(self.armor)
			if self.place.ant.container:
				self.place.remove_insect(self.place.ant.ant)
			else:
				self.place.remove_insect(self)
			return None

		"""throw leaf at nearest bee"""
		ThrowerAnt.action(self, colony)

		"""double damage of ants behind Queen"""
		p = self.place.exit
		while p != None:
			a = p.ant
			if a:
				if a.container:
					if a.ant != None and a.ant not in self.ants:
						a.ant.damage = a.ant.damage * 2
						self.ants.append(a.ant)
				if a not in self.ants:
					a.damage = a.damage * 2
					self.ants.append(a)
			p = p.exit
...
def make_stun(action):
	...
	"""
	# BEGIN Problem EC
	def new_action(colony): #stuns the bee and returns None
		return 
	return new_action
	# END Problem EC
...
def apply_effect(effect, bee, duration):
	...
	# BEGIN Problem EC
	og_action = bee.action  #keep track of original action
	i = 0
	def status_effect(colony): 
		nonlocal i
		if i < duration: #if duration is not 0, apply the effect
			i += 1
			effect(og_action)(colony)
		else:
			og_action(colony) # if duration is 0, return original action
	
	bee.action = status_effect #set action method of bee to the new status effect
	# END Problem EC
