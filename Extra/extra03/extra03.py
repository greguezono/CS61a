...
def all_final_frames(initial_frame, threads):
    ...
    final_frames = []
    for t in threads:
        initial_frame = execute(t.statements[0], initial_frame)
    ...
    def execute_all(frame, threads):
        if not threads:
            ...
            for thread in threads:
                ...
                new_frame = execute_thread(thread, frame)
                if not thread.lock:
                    final_frames.append(new_frame)
                new_threads.remove(thread)
                execute_all(new_frame, new_threads)

    def execute_thread(thread, frame):
        for s in thread.statements:
            frame = execute(s, frame)
        return frame
