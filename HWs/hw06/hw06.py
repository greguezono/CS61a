...
class Fib():
	...
	def next(self):
		if self.value == 0:
			self.previous_value = Fib(1)
		self.n = Fib(self.previous_value.value + self.value)
		self.n.previous_value = self
		return self.n
...
class VendingMachine:
	...
	def vend(self):
		if self.stock == 0 and self.balance == 0:
			...
			previous_balance, self.balance = self.balance, 0
			self.stock -= 1
			return 'Here is your ' + self.product + ' and $' + str(previous_balance-self.price) + ' change.'
		else:
			self.stock -= 1
