...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(procedure, MacroProcedure):
            print(isinstance(procedure, MacroProcedure))
            c = procedure.apply_macro(rest, env)
            return scheme_eval(c, env)
...
def eval_all(expressions, env):
    ...
    # BEGIN PROBLEM 8
    def new_eval(expression, env):
        nonlocal a
        if expression == nil:
            return a
        else:
            a = scheme_eval(expression.first, env)
            return new_eval(expression.second, env)
    if expressions == nil:
        return None
    else:
        a = scheme_eval(expressions.first, env)
        b = expressions
        return new_eval(expressions.second, env)
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("Incorrect number of formals or vals")
        a = formals
        b = vals
        while a != nil:
            f = a.first
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        name = target.first
        a = Pair(target.second, expressions.second)
        b = do_lambda_form(a, env)
        env.define(target.first, b)
        return name
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        operators = Pair(bindings.first.first, operators)
        operands = Pair(scheme_eval(bindings.first.second.first, env), operands)
        bindings = bindings.second
    check_formals(operators)
    return env.make_child_frame(operators, operands)
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    operators = expressions.first.second
    operands = expressions.second.first
    m = MacroProcedure(operators, operands, env)
    env.define(name, m)
    return name
    # END Problem 21
...
class MuProcedure(Procedure):
    ...
    # BEGIN PROBLEM 16
    def make_call_frame(self, args, env):
        a = env.make_child_frame(self.formals, args)
        return a
    # END PROBLEM 16
