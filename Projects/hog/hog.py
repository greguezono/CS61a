...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    player = 0  # Which player is about to take a turn, 0 (first) or 1 (second)    # BEGIN PROBLEM 5
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def announce(s0, s1):
        ...
        if player == 0:
            if score == s0:
                return announce_highest(player, high, score)
            elif high >= s0-score:
                score = score + s0-score
                return announce_highest(player, high, score)
            elif s0-score == 1:
                high = s0
                score = score + high
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    best_turn = 0
    while j > 0:
        averaged_turn_score = make_averaged(roll_dice, 1000)(j, dice)
        if averaged_turn_score > best_turn_score:
            best_turn_score = averaged_turn_score
            best_turn = j
        j-=1
    return best_turn
    # END PROBLEM 9
