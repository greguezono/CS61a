...
def group_by_centroid(restaurants, centroids):
    ...
    lst = [[closest(r), r] for r in restaurants]
...
def find_centroid(cluster):
    ...
    for restaurant in cluster:
        list_of_locations.append(restaurant_location(restaurant))
    for latitude, longitude in list_of_locations:
        list_of_latitudes.append(latitude)
        list_of_longitudes.append(longitude)
    return [mean(list_of_latitudes), mean(list_of_longitudes)]
    # END Question 5
...
def best_predictor(user, restaurants, feature_fns):
    ...
    return max(predictors, key = lambda list: list[1])[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        ...
        if restaurant in reviewed:
            dict[name] = user_rating(user, restaurant_name(restaurant))
        else:
