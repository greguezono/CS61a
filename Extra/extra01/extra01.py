...
def intersect(f, df, g, dg):
    ...
    4.0
    """
    new_function = lambda b: lambda c: lambda x: b(x) - c(x)
    composed_f_and_g = new_function(f)(g)
    return find_zero(composed_f_and_g, df)
...
def roll_at_least_no_ones(total, n, dice):
    ...
    if n == 0 and total > 0:
        ...
        for roll in die_rolls:
            pr += dice(roll) * roll_at_least_no_ones(total-roll, n-1, dice)
        return pr
